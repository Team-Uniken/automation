var webviewObj         = new Object();
webviewObj.proxy       = "";
webviewObj.originalSrc = "";
window.onresize        = doLayout;
var isLoading          = false;
var once               = false;
var loadingPageUrl     = 'file:///' + __dirname + '/loading.html';
loadingPageUrl = loadingPageUrl.replace(/\\/g, "/");
onload = function() {
  ...
  for (var i = 0; i < vars.length; i++)
  {
    ...
    if (decodeURIComponent(pair[0]) == "proxy")
    {
      webviewObj.proxy = decodeURIComponent(pair[1]);
    }
    else if(decodeURIComponent(pair[0]) == "src")
    {
      webviewObj.originalSrc = decodeURIComponent(pair[1]);
    }
  }
  webview.src = loadingPageUrl;
  webview.addEventListener('did-finish-load', function() {
    if(once == false)
    {
      ...
      if(webviewObj.proxy != "")
      {
         ...
         webContents.session.setProxy('http='+webviewObj.proxy+';https='+webviewObj.proxy , function() {
           var webContents = webview.getWebContents();
           console.log('set to webviewPortal Successfully');
         });
      }
      webview.src = webviewObj.originalSrc;
    }
  });
  ...
  document.querySelector('#home').onclick = function() {
    navigateTo(webviewObj.originalSrc);
  };
...
function handleLoadCommit() {
  resetExitedState();
  var webview = document.querySelector('webview');
  var locUrl = webview.getURL();
  if(locUrl == loadingPageUrl)
  {
    locUrl = webviewObj.originalSrc;
  }
  document.querySelector('#location').value = locUrl;
...
function handleLoadAbort(event) {
  var docMsg = 'LoadAbort' + '\n' +
               '  url: ' + event.url + '\n' +
               '  type: ' + event.type + '\n' +
               '  error description: ' + event.errordescription;
  document.write(docMsg);
