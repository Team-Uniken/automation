...
let obj;
let screenId;
const { AsyncStorage, Alert, DeviceEventEmitter, Platform } = ReactNative;

const RDNARequestUtility = require("react-native").NativeModules
  .RDNARequestUtility;
const ReactRdna = require("react-native").NativeModules.ReactRdnaModule;
const onUpdateChallengeStatusModuleEvt = new NativeEventEmitter(
  NativeModules.ReactRdnaModule
);
...
class UpdateAuthMachine extends Component {
  ...
  componentWillMount() {
    ...
    onUpdateChallengeStatusSubscription = onUpdateChallengeStatusModuleEvt.addListener(
    ...
    if (res.errCode == 0) {
      ...
      if (statusCode == 100) {
        if (res.pArgs.response.ResponseData) {
          console.log(
            "UpdateAuthMachine - ResponseData " +
              JSON.stringify(res.pArgs.response.ResponseData)
          );
          const chlngJson = res.pArgs.response.ResponseData;
          const nextChlngName = chlngJson.chlng[0].chlng_name;
          if (chlngJson != null) {
            console.log(
              "UpdateAuthMachine - onCheckChallengeResponseStatus - chlngJson != null"
            );
          ...
          console.log(
            "UpdateAuthMachine - else ResponseData " +
              JSON.stringify(res.pArgs.response.ResponseData)
          );
        ...
        if (
          res.pArgs.response.StatusMsg.toLowerCase().includes("suspended") ||
            ...
            [
              {
                ...
                onPress: () => {
                  ...
                  if (chlngJson != null) {
                    console.log(
                      "UpdateAuthMachine - onUpdateChallengeStatus - chlngJson != null"
                    );
                    //this.props.navigator.immediatelyResetRouteStack(this.props.navigator.getCurrentRoutes().splice(-1, 1));
                    //              this.props.navigator.push({
                    //              id: 'UpdateMachine',
  ...
  showNextChallenge(args) {
    ...
    if (result.challenge) {
      if (result.show === false) {
        while (
          (result = this.canShowNextChallenge()) &&
          result.show === false &&
          result.challenge
        ) {
  ...
  getComponentByName(route, nav) {
    ...
    if (id === "checkuser") {
      ...
      //return (<ScreenHider navigator={nav} url={route.url} title={route.title} />);
      return (
        <DeviceBinding navigator={nav} url={route.url} title={route.title} />
      );
    } else if (id === "ConnectionProfile") {
      return (
        <ConnectionProfile
          navigator={obj.props.navigator}
          url={route.url}
          title={route.title}
        />
      );
      ...
      return (
        <PatternLock
          ...
          onUnlock={route.onUnlock}
          onSetPattern={route.onSetPattern}
          disableClose={route.disableClose}
        />
      );
    }
  ...
  showFirstChallenge(chlngJson, startIndex) {
    ...
    if (chlngJson.chlng && chlngJson.chlng.length > startIndex) {
      ...
      console.log(
        "TwoFactorAuthMachine - onCheckChallengeResponseStatus - chlngJson != null"
      );
      //this.props.navigator.immediatelyResetRouteStack(this.props.navigator.getCurrentRoutes().splice(-1, 1));

      if (
        firstChlngName === "tbacred" ||
        firstChlngName === "devbind" ||
  ...
  canShowNextChallenge() {
    if (this.hasNextChallenge()) {
      ...
      if (
        currChallenge.chlng_name === "tbacred" ||
        ...
        if (currChallenge.chlng_name === "devname") {
          ...
          if (name != null) AsyncStorage.setItem("devname", name);
        }
  ...
  callUpdateChallenge() {
    if (Main.isConnected) {
      ...
      AsyncStorage.getItem("userId")
        .then(value => {
          ReactRdna.updateChallenges(
            JSON.stringify(challengeJson),
            value,
            response => {
